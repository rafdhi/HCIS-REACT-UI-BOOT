...
class moveReport extends Component {
  constructor(props) {
    super(props);
    this.state = {
      auth: props.auth,
  ...
  async downloadReport(value) {
    let res = await fetch(
      process.env.REACT_APP_HCIS_BE_API + "report/rpt/movement.karyawan",
      {
        headers: {
          Authorization:
...
export default connect(mapStateToProps, mapDispatchToProps)(moveReport);
