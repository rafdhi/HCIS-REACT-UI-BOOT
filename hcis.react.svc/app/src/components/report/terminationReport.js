...
class termReport extends Component {
  ...
  async downloadReport(value) {
    let res = await fetch(process.env.REACT_APP_HCIS_BE_API + "report/rpt/terminasi", {
      headers: {
        Authorization:
  ...
  async fileHandler(event) {
    let response = await fetch(process.env.REACT_APP_HCIS_BE_API + "report/rpt/terminasi", {
      headers: {
        Authorization:
          "Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJhZG1pbiJ9.5BG9SEVOGo_xRhtT8IkyoSy60kPg8HM9Vpvb0TdNew4"
      }
...
export default connect(mapStateToProps, mapDispatchToProps)(termReport);
