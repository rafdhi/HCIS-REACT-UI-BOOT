...
class PphReport extends Component {
  constructor(props) {
    super(props);
    this.state = {
      auth: props.auth,
  ...
  async downloadReport(value) {
    let res = await fetch(process.env.REACT_APP_HCIS_BE_API + "report/rpt/pph1721a1", {
      headers: {
        Authorization:
  ...
  async fileHandler(event) {
    let response = await fetch(process.env.REACT_APP_HCIS_BE_API + "report/rpt/pph1721a1", {
      headers: {
        Authorization:
          "Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJhZG1pbiJ9.5BG9SEVOGo_xRhtT8IkyoSy60kPg8HM9Vpvb0TdNew4"
      }
...
export default connect(mapStateToProps, mapDispatchToProps)(PphReport);
