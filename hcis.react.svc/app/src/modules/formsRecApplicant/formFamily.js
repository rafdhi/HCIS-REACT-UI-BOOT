...
import FormAddress from '../formsRecApplicant/formAddress'
...
class formFamily extends Component {
  ...
  addAddress(value, deleteMode = false) {
    ...
    let address = Object.assign([], this.state.payloadFamily.applicantFamilyAddress.address)
    if (value.addressID === "") value.addressID = uuid.v1()
    if (!deleteMode) {
      let isExist = R.findIndex(R.propEq('addressID', value.addressID))(address)

      value.refObjectID = this.state.payloadFamily.applicantFamilyID;
      if (isExist >= 0) {
        ...
        <div className="popup-content background-white border-radius">
          ...
          <form action="#"
            onSubmit={(e) => {
              ...
              payloadFamily = {
                ...payloadFamily,
                applicantFamilyID: payloadFamily.applicantFamilyID === "" ? "F-" + uuid.v1() : payloadFamily.applicantFamilyID,
                familyDivorceDate: payloadFamily.familyDivorceDate === "" ? "" : M(payloadFamily.familyDivorceDate).format("DD-MM-YYYY"),
                familyBirthDate: payloadFamily.familyBirthDate !== "" ? M(payloadFamily.familyBirthDate).format("DD-MM-YYYY") : ""
              }
              ...
              if (!R.isEmpty(payloadFamily.familyKTPNumber) && payloadFamily.familyKTPNumber.length < 16) return alert('KTP Number Must be 16 Digit.')
